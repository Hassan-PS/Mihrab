...
### Added
- **The app tells you what changed after an update.** There was no such screen: the changelog lived on the marketing site and people who already used the app learned what was new by noticing. The feature tour's machinery — the paged, skippable full screen it used to greet new installs with — now carries release notes instead, on the first launch after an update, to the people it is actually for. It is version-aware rather than once-per-install, it catches up somebody who skipped three releases, and a release with nothing worth a slide shows nothing at all, which is the default rather than something anyone has to remember. A fresh install never sees it: they have just been through setup.
- **Mihrab · Sky — the Today hero as an Android widget.** The same drawn sky the app opens on — night with the moon in its phase, dawn, the day, sunset, dusk, from the same model and the same colours — with the next prayer, its time and a live countdown at the foot, the city above and the date below, each line inked black or white by the sky behind it. Placed at 2×2, resizable to 4×3, and drawn correctly at every size in between.
...
### Changed
- **First launch asks the four things that decide whether the app is right for you, and shows you each answer in your own times.** It used to ask one — where you are — across three or four pages of paragraph-and-a-button, then drop you on Home and start a second, four-slide welcome. Meanwhile the two settings that decide whether the app is CORRECT for a person were never raised: the school, so a Ḥanafī user's ʿaṣr was three quarters of an hour early every day until they found it in Settings, and whether the app may notify at all. Setup is now six screens — the salām with the language in the corner, where you are, how you pray, whether we should call you, an optional shelf of things to make it yours, and your own day. The school screen shows your ʿaṣr and moves it as you tap, so nobody has to know what a shadow ratio is to see what the choice does; picking Mālikī offers the second times there and nowhere else. The alerts screen asks the system, writes the setting when the answer is yes, lets you choose an adhan — seventeen ship and the default was the system tone — and folds Android's exact-alarm grant into a row instead of a page of its own. The shelf is skipped in one tap and writes nothing when it is: the advance reminder, sunrise, Islamic midnight and the last third with their times beside them, the morning and evening adhkār, the theme and the accent. The last screen is your city and your times rather than four slides describing them, with the three settings most people change next a tap away. Every answer is written as it is made, so a setup left half-done keeps what you already said; leaving is a decision rather than a swipe; and the step counter is a hairline now. Settings keeps every one of these and now meets you with them already answered — the school sits above the calculation method, where people look for it.
