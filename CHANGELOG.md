...
### Changed
- **The Quran gets itself ready in the background, once.** Each page of the mushaf is drawn with its own typeface, and until now that typeface was fetched the first time you turned to that page — about 300 KB, with the page waiting on it. Read straight through and that is 604 separate waits, always at the moment you wanted to read. The app now quietly collects the whole set in the background and then never fetches again: after a fresh install or an update, whenever the device is on Wi-Fi. It only ever runs while you are on Wi-Fi — the complete set is 180 MB, which has no business landing on a mobile data plan — and it stops the moment you leave, keeping everything it already has, so the next Wi-Fi window carries on from there rather than starting over. There is no progress bar and nothing to confirm; the Quran is readable throughout, it simply stops pausing.
