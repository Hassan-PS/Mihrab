...
- **The sky behind the countdown follows the prayer day.** A drawn sky — night before Fajr, the dawn band before sunrise, a clear morning, the afternoon's warm light, the sunset, dusk — with the sun or moon moving through the hero with the current interval. Drawn as a wash over the hero's own surface so the countdown stays as legible as before, in both themes.
- **Surah names in calligraphy.** The Quran list and the Tilawah page write the Arabic names in Katibeh, a Thuluth-style face (SIL OFL) in the hand mushaf headers are written in, rather than the body Naskh.
