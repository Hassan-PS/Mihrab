...
**v2.7.42 (226) released 2026-07-30** — tag `3465e31`; F-Droid pipeline #2718354944 green first run; verify-release.sh ALL CHECKS PASSED; cask 2.7.42 (sha 884a8952…). Driven by MR 36312 review round 3: linsui asked for the **TetheredNet** anti-feature (added as a MAP with a SHORT one-line en-US reason — long prose makes `fdroid rewritemeta` re-wrap and leave TRAILING SPACES that yamllint then rejects; keep the reason under ~68 chars) and a tester (@tunefriend) LGTM+nits → (a) WRITE/READ_EXTERNAL_STORAGE were injected by react-native-blob-util manifest merging, never ours (app writes only to DocumentDir) → stripped via `tools:node="remove"`, verified with aapt2 + a real mushaf download; (b) store description rewritten (old name/stale Qibla tile removed per user decision, explicit optional-network disclosure). Also folded in the macOS menu-bar fix: **CFBundleName was $(PRODUCT_NAME) = "PrayerApp"** — macOS uses it for the app menu while CFBundleDisplayName only covers the iOS home screen. NOTE: `glab` OAuth token is EXPIRED (needs interactive `glab auth login`); MR reading/posting went through the Chrome MCP, and the plain `…/notes` API URL (no query string) works while query-string URLs get blocked by the extension.
